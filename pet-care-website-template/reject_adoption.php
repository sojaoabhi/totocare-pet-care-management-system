...
if ($id) {
    try {
        ...
        $sql = "UPDATE adoption_requests SET status = 'Rejected' WHERE id = :id";
        ...
        if ($adopter) {
            ...
            $mail->Body = "Dear " . htmlspecialchars($adopter['name']) . ",<br>We regret to inform you that your adoption request has been rejected.";

            if ($mail->send()) {
                echo "Adoption request rejected and email sent!";
            } else {
                echo "Adoption request rejected, but email could not be sent.";
            }
        }
